# Standardize the colors of the 11 masks and put them in order
...
# Notes:
#   - The order it's put in does *not* reflect the //final// tissue order
#   - Ordering is meant to organize masks for CSF/final hole checking
#   - Execute (XYZ) for final mask orders
#   - Order:
...
# Mask organization
App.GetDocument().CreateMask("----------------------", Colour(255, 0, 0))
App.GetDocument().GetGenericMaskByName("----------------------").Activate()
...
App.GetDocument().GetGenericMaskByName("cancellous").Activate()
App.GetDocument().MoveMaskTo(App.GetDocument().GetActiveGenericMask(), 14)
